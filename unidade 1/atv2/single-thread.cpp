...
desvio padrão. A implementação utiliza uma única thread, executando
todos os cálculos de forma sequencial. O propósito é servir como 
baseline de comparação com as versões que utilizam múltiplas threads.
O programa mede o tempo total de processamento para análise de 
desempenho.
...
double calc_mediana() {
    // Cria cópia local para realizar ordenação (evita modificar vetor original)
...
int main() {
    ...
    cout << "Exemplo de execucao com 1 thread (single-thread)" << endl;
